<script lang="ts">
    ...
    let { fields, loading, initialData, onClose, onSave }: Props = $props();

    // Initialize with clone of initialData
    let formData = $state<Record<string, any>>(initialData ? { ...initialData } : {});
    
    // Filter out system fields (but keep them in data if editing? No, we don't edit them)
    // We only display editable fields.
    ...
    let isEdit = $derived(!!initialData);
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        <div class="flex items-center justify-between p-4 border-b border-zinc-800">
            <h2 class="text-lg font-semibold text-zinc-100">{isEdit ? 'Edit Record' : 'Insert Record'}</h2>
